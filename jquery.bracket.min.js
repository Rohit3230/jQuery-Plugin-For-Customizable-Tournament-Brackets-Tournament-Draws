...
! function(a) { 
    ...
    function i(a, b, c, d) {
        // if(c === "555"){
        //     console.log('main result---',c);
        //     return void a.append("Match Cancled");
        // }else{
            switch (d) {
                case "empty-bye":
                    return void a.append("BYE");
                case "empty-tbd":
                    return void a.append("-");
                case "entry-no-score": 
    ...
    function r(c, d, e, f, g, h, i, j, k, l) {
        ...
        var m = e.name.isEmpty() || f.name.isEmpty() ? "" : 'data-resultid="result-' + j.getNext() + '"',
            ...
            o = (e.score.val != -4 && f.score.val != -4) && (e.name.isEmpty() || f.name.isEmpty() || !g) ? u.empty() : e.score.map(function(a) {
                return "" + a
            }),
            p = o.orElse("-");
            var isScoreMatched = false; 
            if(i.init.displayResults){
                // code by rohit singh niranjan for dispalying text sting on score place corner of the div instead of current score.
                if(!e.name.isEmpty()){
                    var initObj = i.init;
                    (initObj.displayResults).forEach(function (value, key){
                        var scoreVal = value; 
                          
                           if(
                               d.a.score.val === scoreVal.teamScore[0]
                               && d.b.score.val === scoreVal.teamScore[1]
                               && d.a.name.val === scoreVal.team1Name
                               && d.b.name.val === scoreVal.team2Name
                            ){ 
        
                                        if(!scoreVal.isInfoPrinted){ 
                                                //n[1].innerHTML = $($.parseHTML(scoreVal.teamValue[0]));
                                                 n.html($($.parseHTML(scoreVal.teamValue[0]))); 
                                                isScoreMatched = true;
                                                scoreVal.isInfoPrinted = true;
                                        } 
                            } 
        ...
        var q = e.name.map(function() {
            ...
            function getTextValue(scoreText){
                if(scoreText == '-1' || scoreText == '-2'){
                    return 'No Match'
                }else if(scoreText == '-4'){
                    return 'W.O.';
                }else{
                    return e.name.toNull();
                }                
            };

            function getTeamInfo(teamsObj ,displayResults){ 
                var str;
                for(var i = 0; i<= displayResults.length-1 ; i++){
                    var resultObj = displayResults[i];
                    if(
                        resultObj.teamInfo
                        && resultObj.teamInfo.team1Info
                        && e.name.val == resultObj.team1Name
                        && resultObj.matchSerial
                        && resultObj.matchSerial == matchNumber
                        // && teamsObj.a.name.val == resultObj.team1Name
                        // && teamsObj.b.name.val == resultObj.team2Name
                    ){
                        str = resultObj.teamInfo.team1Info;
                        break;
                    }
                    if(
                        resultObj.teamInfo
                        && resultObj.teamInfo.team2Info
                        && e.name.val == resultObj.team2Name
                        && resultObj.matchSerial
                        && resultObj.matchSerial == matchNumber
                        // && teamsObj.a.name.val == resultObj.team1Name
                        // && teamsObj.b.name.val == resultObj.team2Name
                    ){
                        str = resultObj.teamInfo.team2Info;
                        break;
                    }
                } 
                return str;
                ...
                if(
                    i
                    && i.init
                    && i.init.displayResults
                ){
                    (i.init.displayResults).forEach(function (value, key){
                            var scoreVal = value;  

                                if(
                                    scoreVal.roundValue
                                    && scoreVal.roundValue.roundString 
                                ){  
                                    if(
                                        $("#yoGemsRound"+scoreVal.roundValue.roundIndex)
                                        && $("#yoGemsRound" + scoreVal.roundValue.roundIndex)[0]
                                        && ($("#yoGemsRound" + scoreVal.roundValue.roundIndex)[0].children)
                                        && ($("#yoGemsRound" + scoreVal.roundValue.roundIndex)[0].children).length == 0
                                    ){
                                        $("#yoGemsRound"+scoreVal.roundValue.roundIndex).append("<div class='roundName'>"+scoreVal.roundValue.roundString+"</div>");
                                        if(scoreVal.roundValue.roundIndex == 0 && i.init.schoonHeading){
                                           $("#yoGemsRound"+scoreVal.roundValue.roundIndex).append("<div class='schoolNameRow'>"+i.init.schoonHeading+"</div>"); 
                                           $(".jQBracket").addClass('shiftDraw'); 
                                        }
                                    }
        ...
        if (i.decorator.render(s, getTextValue(p), p, q), e.seed.forEach(function(a) {
            ...
            e.name.isEmpty() ?
                r.addClass("na") 
                :
                d.winner().name === e.name
                    ? r.addClass("win")
                    :
                    d.loser().name === e.name && r.addClass("lose"),
             r.append(n),
             r.append((c == 0 && getTeamInfo(d ,i.init.displayResults) && getTeamInfo(d ,i.init.displayResults) != 'BYE' ? "<div class='userSchoolName'>"+getTeamInfo(d ,i.init.displayResults)+"</div>" : '')),
             (!e.name.isEmpty() || e.name.isEmpty() && 0 === c && h) && "function" == typeof i.save && (i.disableTeamEdit || (s.addClass("editable"), s.click(function() {
                function b() {
                    function f(f, g) {
                        var h = e.seed.get();
            ...
            n.addClass("editable"), n.click(function() {
                function c() {
                    // Disable code by Rohit for make non-editable score field.
                    // d.unbind();
        ...
        // code for reinitialize matchNumber so that it can be re use for another draw.
        if(
            i
            && i.init
            && i.init.displayResults
            && i.init.displayResults.length
            && i.init.displayResults.length == matchNumber
        ){
            matchNumber = 0;
        }
        return r
    ...
    var x = function() {
        ...
        E = function() {
            function b(b, c, d, e, f, g, h, i) {
                this.bracket = b,
                this.previousRound = c,
                 this.roundNumber = d,
                  this._results = e,
                   this.doRenderCb = f,
                    this.mkMatch = g,
                     this.isFirstBracket = h,
                      this.opts = i,
                       this.containerWidth = this.opts.teamWidth + this.opts.scoreWidth,
                        this.roundCon = a('<div class="round" id="yoGemsRound'+d+'" style="width: ' + this.containerWidth + "px; margin-right: " + this.opts.roundMargin + 'px"/>'),
                         this.matches = []
            }
        ...
        H = function() {
            ...
            return Object.defineProperty(c.prototype, "el", {
                ...
                // (this.match.a.name.val ? this.match.a.name.val : this.match.b.name.val)
                if(matchNumber > 0){this.teamCon.append('<div class="matchNumber" style="margin-top: -19px;">#'+matchNumber+'</div>');}
